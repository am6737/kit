...
func (m *RemoteConfigManager) Get(key string, keys ...string) (*config.Config, error) {
	ac := &config.Config{}
...
// Watch 监听配置项的变化并更新配置
func (m *RemoteConfigManager) Watch(ac *config.Config, updateCh chan<- ConfigUpdate, key string, keys ...string) error {
	//m.once.Do(func() {
	go func(k string) {
...
func (m *RemoteConfigManager) handlerAppConfig(ac *config.Config, newValue string) error {
	// 解析新的配置值
...
func (m *RemoteConfigManager) handlerConfig(key string, ac *config.Config, newValue string) error {
...
func updateConfigField(fieldName string, ac *config.Config, newConfig map[string]interface{}) error {
...
func (m *RemoteConfigManager) saveConfigToFile(ac *config.Config) error {
	m.configLocker.Lock()
...
func LoadDefaultRemoteConfig(addr string, configName string, tokens string, ac *config.Config) (chan ConfigUpdate, error) {
